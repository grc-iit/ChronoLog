...
//    std::shared_ptr<std::unordered_map<std::string, Chronicle *>> getChronicleMap() { return chronicleMap_; }
    ...
    int acquire_chronicle(const std::string &client_id, const std::string& name, int& flags);
    int release_chronicle(const std::string &client_id, const std::string& name, int& flags);
