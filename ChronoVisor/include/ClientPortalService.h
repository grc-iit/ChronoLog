...
public:
    ...
    {
        int return_code = theVisorClientPortal.CreateChronicle(client_id, chronicle_name, attrs);
        request.respond(return_code);
    }
                      ...
                      const std::map<std::string, std::string>& attrs)
    {
        AcquireStoryResponseMsg acquire_response =
                theVisorClientPortal.AcquireStory(client_id, chronicle_name, story_name, attrs);
