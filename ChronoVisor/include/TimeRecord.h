...
public:
    ...
    TimeRecord(Clocksource*clocksource, uint64_t timestamp, double ref_interval): timestamp_(timestamp), ref_interval_(
            ref_interval), clocksource_(clocksource)
    {}
    ...
    { ref_interval_ = update_interval; }

    void setTimestamp(uint64_t timestamp)
    { timestamp_ = timestamp; }

    void setDriftRate(double drift_rate)
