...
namespace chronolog {
    ...
    int KeeperRegistry::InitializeRegistryService(ChronoLog::ConfigurationManager const &confManager) {
        ...
        if (registryState != UNKNOWN) { return chronolog::CL_SUCCESS; }
...
/////////////////
    ...
    int KeeperRegistry::ShutdownRegistryService() {
        ...
        if (is_shutting_down()) { return chronolog::CL_SUCCESS; }
        ...
        if (!keeperProcessRegistry.empty()) {
            for (auto process: keeperProcessRegistry) {
                if (process.second.keeperAdminClient != nullptr) {
                    std::cout << "KeeperRegistry: sending shutdown to keeper {" << process.second.idCard << "}"
                              << std::endl;
        ...
        if (nullptr != keeperRegistryService) { delete keeperRegistryService; }
    ...
    int KeeperRegistry::registerKeeperProcess(KeeperRegistrationMsg const &keeper_reg_msg) {
        ...
        if (keeper_process_iter != keeperProcessRegistry.end()) {
            ...
            if ((*keeper_process_iter).second.keeperAdminClient !=
                nullptr) { delete (*keeper_process_iter).second.keeperAdminClient; }
        ...
        if (keeperProcessRegistry.size() > 0) { registryState = RUNNING; }

        std::cout << "KeeperRegistry : RUNNING with {" << keeperProcessRegistry.size() << "} KeeperProcesses"
                  << std::endl;
...
/////////////////
    ...
    void KeeperRegistry::updateKeeperProcessStats(KeeperStatsMsg const &keeperStatsMsg) {
        ...
        auto keeper_process_iter = keeperProcessRegistry.find(
                std::pair<uint32_t, uint16_t>(keeper_id_card.getIPaddr(), keeper_id_card.getPort()));
        if (keeper_process_iter == keeperProcessRegistry.end()) {    // however unlikely it is that the stats msg would be delivered for the keeper that's already unregistered
...
/////////////////
    ...
    std::vector<KeeperIdCard> &KeeperRegistry::getActiveKeepers(
            std::vector<KeeperIdCard> &keeper_id_cards) {  //the process of keeper selection will probably get more nuanced;
        //for now just return all the keepers registered
        if (is_shutting_down()) { return keeper_id_cards; }
...
/////////////////
    ...
    int KeeperRegistry::notifyKeepersOfStoryRecordingStart(std::vector<KeeperIdCard> const &vectorOfKeepers,
        ...
        std::lock_guard<std::mutex> lock(registryLock);
        if (!is_running()) { return chronolog::CL_ERR_NO_KEEPERS; }
