...
def reader_client():
    ...
    clientQueryConf = py_chronolog_client.ClientQueryServiceConf(
        "ofi+sockets", "127.0.0.1", 5557, 57
    )
    ...
    attrs = dict()
    return_tuple = reader_client.AcquireStory("py_chronicle", "my_story", attrs)
    print("\n client.AcquireStory() returned:", return_tuple)

    event_series = py_chronolog_client.EventList()
    return_code = reader_client.ReplayStory(
        "py_chronicle",
        "my_story",
        1,
        2000000000000000000,
    ...
    print(
        "\n client.ReplayStory() call returns event_series : ",
        event_series,
        "with ",
        len(event_series),
        " events",
