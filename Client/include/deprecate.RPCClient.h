...
public:
    ...
    {
        LOGD("%s is called in PID=%d, with args: client_id=%s, chronicle_name=%s, story_name=%s, flags=%d", __FUNCTION__
    ...
    {
        LOGD("%s is called in PID=%d, with args: client_id=%s, chronicle_name=%s, story_name=%s", __FUNCTION__, getpid()
