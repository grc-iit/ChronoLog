...
public:
    ...
    template <typename T, typename N>
    void
    create_keyvalues_ordered(int s_id, std::vector <N> &keys, std::vector <std::string> &values, std::vector <int> &ops
                             , int rate)
    {
    ...
    template <typename T, typename N>
    void create_keyvalues(int s_id, std::vector <N> &keys, std::vector <std::string> &values, std::vector <int> &ops
                          , int rate)
    {
        std::string s = k_args[s_id].tname;
        std::string attr_name = k_args[s_id].attr_name;
    ...
    void
    get_ycsb_timeseries_workload(std::string &, std::vector <uint64_t> &, std::vector <float> &, std::vector <int> &);
    ...
    int spawn_kvstream(std::string &s, std::string &a, int maxsize, int n1, int n2, int n3)
    {
        ...
        std::function <void(struct kstream_args*)> KVStream(
                std::bind(&KeyValueStore::cacheflushInvList <T, N>, this, std::placeholders::_1));
    ...
    int start_session(std::string &name, std::string &attrname, KeyValueStoreMetadata &m, int maxsize, int n1, int n2
                      , int n3)
    {
