...
public :
    KeyValueStoreAccessor(int np, int p, KeyValueStoreMetadata &m, KeyValueStoreIO*io, Interface_Queues*ifq
    ...
    int create_invertedlist(std::string &attr_name, int c, int maxsize)
    {
        ...
        if(r == secondary_attributes.end())
        {
            ...
            if(type.compare("int") == 0)
            {
                ...
                ret = add_new_inverted_list <integer_invlist, int>(name, attr_name, maxsize, numtables, maxint, d, kio
                                                                   , if_q, c, md.value_size());
            }
            else if(type.compare("unsignedlong") == 0)
            {
                uint64_t maxuint = UINT64_MAX;
                ret = add_new_inverted_list <unsigned_long_invlist, uint64_t>(name, attr_name, maxsize, numtables
                                                                              , maxuint, d, kio, if_q, c
                                                                              , md.value_size());
            }
            else if(type.compare("float") == 0)
            {
                float maxfl = DBL_MAX;
                ret = add_new_inverted_list <float_invlist, float>(name, attr_name, maxsize, numtables, maxfl, d, kio
                                                                   , if_q, c, md.value_size());
            }
            else if(type.compare("double") == 0)
            {
                double maxd = DBL_MAX;
                ret = add_new_inverted_list <double_invlist, double>(name, attr_name, maxsize, numtables, maxd, d, kio
                                                                     , if_q, c, md.value_size());
            }
        }
        else ret = r->second;
