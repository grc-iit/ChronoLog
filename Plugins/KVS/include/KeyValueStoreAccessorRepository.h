...
public:
    KeyValueStoreAccessorRepository(int np, int p, KeyValueStoreIO*io_layer, Interface_Queues*ifq
                                    , data_server_client*ds): numprocs(np), myrank(p)
    {
        io = io_layer;
        if_q = ifq;
        d = ds;
        ...
        std::string emptykey;
        accessor_maps = new BlockMap <std::string, KeyValueStoreAccessor*, stringhash, stringequal>(128, t_pool
                                                                                                    , emptykey);
    }
    ...
    bool create_invertedlist(std::string &s, std::string &a, int c, int maxsize)
    {
        ...
        int ret = ka->create_invertedlist(a, c, maxsize);
