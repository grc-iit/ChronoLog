...
public :
    ...
    KeyValueStoreMDS(int np, int r): numprocs(np), myrank(r)
    {
        ...
        if(serverid == 0)
        {
            ...
            std::string emptykey = "";
            directory = new BlockMap <std::string, KeyValueStoreMetadata*, stringhash, stringequal>(1024, t_pool
                                                                                                    , emptykey);
        }
        else
        {
    ...
    void bind_functions()
    {
        ...
        std::function <void(const tl::request &, std::string &, std::vector <std::string> &)> insertFunc(
                std::bind(&KeyValueStoreMDS::ThalliumLocalInsert, this, std::placeholders::_1, std::placeholders::_2
                          , std::placeholders::_3));
