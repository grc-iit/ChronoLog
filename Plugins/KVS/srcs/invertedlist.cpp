...
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::get_events()
{
    ...
    if(exists_t == numprocs)
    {
        ...
        H5Tinsert(kv1, "key", HOFFSET(
        struct KeyIndex <KeyT>,key),datatype);
        H5Tinsert(kv1, "index", HOFFSET(
        struct KeyIndex <KeyT>,index),H5T_NATIVE_UINT64);
...
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::flush_timestamps(int offset, bool persist)
{
    ...
    if(index_writes == 0)
    {
        ...
        std::vector <struct KeyIndex <KeyT>> mergeKeyTimestamps = merge_keyoffsets(preKeyTimestamps, KeyTimestamps
                                                                                   , numkeys_n);
...
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::flush_table_file(int offset, bool persist)
{
...

    {
        ...
        if(sum_v == numprocs)
        {
            ...
            std::vector <struct KeyIndex <KeyT>> mergeKeyTimestamps = merge_keyoffsets(preKeyTimestamps, KeyTimestamps_s
                                                                                       , numkeys_n);
...
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::create_index_file()
{
    ...
    std::string d_string = attributename;
    std::string dtable = attributename + "attr";
    hid_t dataset_t = H5Dcreate(fid, dtable.c_str(), H5T_NATIVE_INT, file_dataspace_t, H5P_DEFAULT, dataset_pl
                                , H5P_DEFAULT);
...
hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::merge_keyoffsets(std::vector <struct KeyIndex <KeyT>> &prevlist
                                                                 , std::vector <struct KeyIndex <KeyT>> &newlist
...
template <typename KeyT, typename ValueT, typename hashfcn, typename equalfcn>
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::add_entries_to_tables(std::string &s
                                                                           , std::vector <struct keydata>*buffer
                                                                           , uint64_t f_offset, int offset)
{
...
template <typename KeyT, typename ValueT, typename hashfcn, typename equalfcn>
void hdf5_invlist <KeyT, ValueT, hashfcn, equalfcn>::get_entries_from_tables(
        std::vector <struct KeyIndex <KeyT>> &KeyTimestamps, int &key_b, int &numkeys, uint64_t maxts)
{
