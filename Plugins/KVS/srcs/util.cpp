...
void create_integertestinput(std::string &name, int numprocs, int myrank, int offset, std::vector <int> &keys
...
void create_ycsb_input(std::string &filename, int numprocs, int myrank, std::vector <uint64_t> &keys
                       , std::vector <std::string> &values, std::vector <int> &op)
{
...
void create_timeseries_testinput(std::string &filename, int numprocs, int myrank, std::vector <uint64_t> &keys
                                 , std::vector <float> &values, std::vector <int> &op)
{
    if(myrank == 0)
    {
        std::ifstream ist(filename.c_str(), std::ios_base::in);
...
void create_dataworld_testinput(std::string &filename, int numprocs, int myrank, std::vector <uint64_t> &keys
                                , std::vector <uint64_t> &ts, std::vector <int> &op)
{
    hid_t xfer_plist = H5Pcreate(H5P_DATASET_XFER);
    hid_t fapl = H5Pcreate(H5P_FILE_ACCESS);
    H5Pset_fapl_mpio(fapl, MPI_COMM_WORLD, MPI_INFO_NULL);
