...
bool dsort::sort_data(int index, int tag, int size, uint64_t &min_v, uint64_t &max_v, event_metadata &em)
{
    ...
    if(num_splitters > 0)
    {
        ...
        for(int i = 0; i < numprocs; i++)
        {
            ...
            {
                MPI_Isend(mysplitters.data(), splitter_counts[myrank], MPI_UINT64_T, i, tag, MPI_COMM_WORLD
                          , &reqs[nreq]);
                nreq++;
            }
        }
        ...
        for(int i = 0; i < numprocs; i++)
        {
            if(send_counts[i] > 0)
            {
                MPI_Isend(&send_buffer[send_displ[i]], send_counts[i] * keyvaluesize, MPI_CHAR, i, tag, MPI_COMM_WORLD
                          , &reqs[nreq]);
                nreq++;
            }
            if(recv_counts[i] > 0)
            {
                MPI_Irecv(&recv_buffer[recv_displ[i]], recv_counts[i] * keyvaluesize, MPI_CHAR, i, tag, MPI_COMM_WORLD
                          , &reqs[nreq]);
                nreq++;
            }
