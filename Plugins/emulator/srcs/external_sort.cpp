...
template <typename T>
std::string hdf5_sort::sort_on_secondary_key(std::string &s1_string, std::string &attr_name, int offset, uint64_t minkey
                                             , uint64_t maxkey, std::string &attr_type)
{
    ...
    for(int i = 0; i < blockids.size(); i++)
    {
        ...
        if(i == 0)
        {
            ...
            hid_t dataset2 = H5Dcreate(fid2, data_string.c_str(), s2, file_dataspace2, H5P_DEFAULT, dataset_pl
                                       , H5P_DEFAULT);
            ...
            hid_t attrid2 = H5Acreate(dataset2, attrname[0], H5T_NATIVE_UINT64, attr_space[0], H5P_DEFAULT
                                      , H5P_DEFAULT);
...
void
hdf5_sort::sort_block_secondary_key(std::vector <struct event>*events, int tag, int offset, T &min_value, T &max_value
                                    , int &offset_f2)
{
    ...
    while(true)
    {
        ...
        for(int j = 0; j < nrecords.size(); j += 2)
        {
            ...
            while(nrecords_t1 > 0)
            {
                ...
                if(std::is_same <T, int>::value)
                    nrecordsw = insert_block <int, MPI_INT>(block1, block2, sorted_block, offset, tag, w_offset
                                                            , minkey_a, maxkey_a);
...
template <typename T, int M>
int hdf5_sort::insert_block(std::vector <struct event>*block1, std::vector <struct event>*block2
                            , std::vector <struct event>*sorted_vec, int offset, int tag, int &offset_w, T &minkey_a
                            , T &maxkey_a)
{
...
template <typename T, int M>
void hdf5_sort::count_offset(std::vector <struct event>*block1, std::vector <struct event>*block2, int &total_records
                             , int offset_a, int &offset, int tag, T &maxkey, bool &block_id)
{
