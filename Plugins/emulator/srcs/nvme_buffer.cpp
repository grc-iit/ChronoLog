...
int nvme_buffers::get_proc(std::string &s, uint64_t ts)
{
    ...
    for(int i = 0; i < nvme_intervals[index].size(); i++)
    {
        ...
        {
            if(ts >= nvme_intervals[index][i][j].first && ts <= nvme_intervals[index][i][j].second)
            {
                pid = j;
                found = true;
                break;
...
void nvme_buffers::fetch_buffer(std::vector <char>*data_mem, std::string &s, int &index, int &tag, int &bc
                                , std::vector <std::vector <int>> &blockcounts)
{
