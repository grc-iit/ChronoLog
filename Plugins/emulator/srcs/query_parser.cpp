...
bool query_parser::sort_events_by_attr(std::string &s, std::vector<struct event> &r_events, std::string &a_name,event_metadata &em,std::vector<view>&resp)
{
   bool b; 
   int offset; 
...
bool query_parser::select_by_attr(std::string &s,std::string &a_name,std::vector<struct event>& r_events,event_metadata &em,T &value,std::vector<view> &resp)
{
	...
	{
	    view ep;
	    ep.offset = offset; ep.length = size;
	    ep.e = r_events[i];
	    resp.push_back(ep);
	}
    }
...
bool query_parser::create_view_from_events(std::string &s,std::vector<struct event>&r_events,std::vector<view> &resp)
{
