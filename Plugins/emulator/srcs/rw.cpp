...
std::vector <uint64_t> read_write_process::add_event(std::string &s, std::string &data)
{
    ...
    if(data.length() != datasize)
        std::cout << " data length = " << data.length() << " datasize = " << datasize << std::endl;
...
void read_write_process::spawn_write_streams(std::vector <std::string> &snames, std::vector <int> &total_events
                                             , int nbatches)
{
...
void read_write_process::pwrite_extend_files(std::vector <std::string> &sts, std::vector <hsize_t> &total_records
                                             , std::vector <hsize_t> &offsets, std::vector <
        std::pair < std::vector <struct event> * , std::vector < char>*

>>&data_arrays,
std::vector <uint64_t> &minkeys, std::vector <uint64_t>
&maxkeys,
bool clear_nvme, std::vector <int>
&bcounts,std::vector <std::vector <std::vector < int>>>&blockcounts)
{
hid_t fid;
hid_t acc_tpl;
hid_t xfer_plist;
...
HOFFSET(struct event, ts), H5T_NATIVE_UINT64

);
H5Tinsert(s2,
"data",

HOFFSET(struct event, data), s1
...
while(nm->
get_buffer(nm_index, tag_p,
2)==false);
nm->
erase_from_nvme(sts[d], data_arrays[d]
.second->

size()

/keyvaluesize,bcounts[d]);
nm->
release_buffer(nm_index);
}}
if(data_arrays[d].second != nullptr) delete data_arrays[d].
second;
prefix += bcounts[d];
}
...
bool read_write_process::pread(std::vector <std::vector <struct io_request*>> &my_requests, int maxstreams)
{
    ...
    for(int i = 0; i < my_requests.size(); i++)
    {
        ...
        if(file_exists_t == numprocs)
        {
            ...
            struct event,ts),H5T_NATIVE_UINT64);
            H5Tinsert(s2, "value", HOFFSET(
            struct event,data),s1);
            ...
            if(em.get_invlist())
            {
                ...
                H5Tinsert(s11, "ts", HOFFSET(
                struct ts_offset,ts),H5T_NATIVE_UINT64);
                H5Tinsert(s11, "offset", HOFFSET(
                struct ts_offset,offset),H5T_NATIVE_UINT64);
                ...
                ret = H5Dread(dataset_t, H5T_NATIVE_INT, mem_dataspace, filedataspace_t, xfer_plist
                              , table_offset.data());
                ...
                send_counts.resize(numprocs);
                recv_counts.resize(numprocs);
                send_displ.resize(numprocs);
                recv_displ.resize(numprocs);
                ...
                MPI_Alltoallv(send_buffer.data(), send_counts.data(), send_displ.data(), MPI_UINT64_T
                              , recv_buffer.data(), recv_counts.data(), recv_displ.data(), MPI_UINT64_T
                              , MPI_COMM_WORLD);
            ...
            for(int n = 0; n < my_requests[i].size(); n++)
            {
                ...
                for(int j = 0; j < numblocks; j++)
                {
                    ...
                    if(mints >= bmin && mints <= bmax || maxts >= bmin && maxts <= bmax || mints < bmin && bmax < maxts)
...
std::pair <std::vector <struct event> * , std::vector < char>*>
read_write_process::create_data_spaces(std::string
&s,
hsize_t &poffset, hsize_t
&trecords,
uint64_t &minkey, uint64_t
&maxkey,
bool from_nvme,
int &nblocks, std::vector <std::vector < int>>
&blockcounts)
{

std::vector <int> num_events_recorded_l, num_events_recorded;
num_events_recorded_l.
resize(numprocs);
num_events_recorded.
resize(numprocs);
std::fill(num_events_recorded_l
.

begin(), num_events_recorded_l
...
0);
std::fill(num_events_recorded
.

begin(), num_events_recorded
...
3)==false);
nm->
fetch_buffer(datamem, s, index, tag_p, nblocks, blockcounts
);
...
num_events_recorded_l[myrank] = datamem->
...
MPI_Allreduce(num_events_recorded_l
.

data(), num_events_recorded

.

data(), numprocs, MPI_INT, MPI_SUM, MPI_COMM_WORLD
...
for(
int j = 0;
j <num_events_recorded.

size();

j++) total_records += (hsize_t)num_events_recorded[j];
...
MPI_Allreduce(&min_key, &minkey,
1,MPI_UINT64_T,MPI_MIN,MPI_COMM_WORLD);

MPI_Allreduce(&max_key, &maxkey,
1,MPI_UINT64_T,MPI_MAX,MPI_COMM_WORLD);

if(myrank==0) std::cout <<" total_records = "<<total_records<<" minkey = "<<minkey<<" maxkey = "<<maxkey<<
std::endl;
...
void read_write_process::pwrite_files(std::vector <std::string> &sts, std::vector <hsize_t> &total_records
                                      , std::vector <hsize_t> &offsets, std::vector <
        std::pair < std::vector <struct event> * , std::vector < char>*
...
int k = 0;
k <numprocs;
k++) blocktotal += blockcounts[i][j][k];
...
hid_t attr_id[1];
attr_id[0] =
H5Acreate(dataset1, attr_name[0], H5T_NATIVE_UINT64, attr_space[0], H5P_DEFAULT, H5P_DEFAULT
);
...
void read_write_process::create_inverted_list(std::string &s, std::vector <std::vector <int>> &bcounts, std::pair <
        std::vector <struct event> * , std::vector < char>*
...
HOFFSET(struct ts_offset, ts), H5T_NATIVE_UINT64

);
H5Tinsert(s11,
"offset",

HOFFSET(struct ts_offset, offset), H5T_NATIVE_UINT64
...
send_buffer[displ] = timestamp_offsets[i].
first;
displ++;
send_buffer[displ] = timestamp_offsets[i].
second;
...
int i = 1;
i <numprocs;
i++) send_displ[i] = send_displ[i-1]+send_counts[i-1];

total_size_s = 0;
for(
int i = 0;
i <numprocs;
i++) total_size_s += recv_counts[i];
...
int i = 1;
i <numprocs;
i++) recv_displ[i] = recv_displ[i-1]+recv_counts[i-1];
...
data(), MPI_UINT64_T, MPI_COMM_WORLD
...
MPI_Allgather(&total_records_l,
1,MPI_INT,recv_values.
...
int i = 0;
i <numprocs;
i++) totalrecords += recv_values[i];

hsize_t offset_inv = 0;
for(
int i = 0;
i <myrank;
i++) offset_inv += recv_values[i];
...
hid_t dataset_t = H5Dcreate(fid, da_string.c_str(), H5T_NATIVE_INT, filedataspace_t, H5P_DEFAULT, dataset_pl
                            , H5P_DEFAULT);
...
void read_write_process::merge_inverted_list(std::string &s, std::vector <std::vector <int>> &bcounts, std::pair <
        std::vector <struct event> * , std::vector < char>*
...
int i = 0;
i <maxsize;
i++) prev_total_l += prev_table[2*i];

int prev_total = 0;
MPI_Allreduce(&prev_total_l, &prev_total,
...
hsize_t offset_prev = (hsize_t)
prev_table[1];
hsize_t blocksize = (hsize_t)
prev_total_l;
...
void read_write_process::pwrite(std::vector <std::string> &sts, std::vector <hsize_t> &total_records
                                , std::vector <hsize_t> &offsets, std::vector <
        std::pair < std::vector <struct event> * , std::vector < char>*
...
std::vector <std::pair < std::vector <struct event> * , std::vector < char>*>> darray_n,
darray_e;
...
push_back(sts[i]);
trec_n.
push_back(total_records[i]);
off_n.
push_back(offsets[i]);
darray_n.
push_back(data_arrays[i]);
minkeys_n.
push_back(minkeys[i]);
maxkeys_n.
push_back(maxkeys[i]);
...
void read_write_process::io_polling(struct thread_arg_w*t)
{
    ...
    while(true)
    {
        ...
        if(active_valid_stream > 0)
        {
            ...
            MPI_Allreduce(pending_streams.data(), active_reqs.data(), active_valid_stream, MPI_INT, MPI_SUM
                          , MPI_COMM_WORLD);
            ...
            for(int i = 0; i < write_reqs.size(); i++)
            {
                ...
                if(r != nullptr)
                {
                    ...
                    try
                    {
                        data_rp = create_data_spaces(r->name, offset, trecords, min_key, max_key, true, nblocks
                                                     , blockc);
                    }
                    catch(const std::exception &except)
                    {
