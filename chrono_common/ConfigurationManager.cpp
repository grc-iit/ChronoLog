...
void ChronoLog::ConfigurationManager::parseGrapherConf(json_object*json_conf)
{
       json_object_object_foreach(json_conf, key, val)
        {
            if(strcmp(key, "RecordingService") == 0)
            {
                assert(json_object_is_type(val, json_type_object));
                json_object* recording_service_conf = json_object_object_get(json_conf, "RecordingService");
                json_object_object_foreach(recording_service_conf, key, val)
                {
                    if(strcmp(key, "rpc") == 0)
                    {
                        parseRPCProviderConf(val, GRAPHER_CONF.RECORDING_SERVICE_CONF);
                    }
                    else
                    {
                        std::cerr << "[ConfigurationManager] [chrono_grapher] Unknown RecordingService configuration: " << key
                                  << std::endl;
                    }
                }
            }
            else if(strcmp(key, "DataStoreAdminService") == 0)
            {
                ...
                json_object_object_foreach(keeper_data_store_admin_service_conf, key, val)
                {
                    ...
                    {
                        std::cerr << "[ConfigurationManager] [chrono_grapher] Unknown DataStoreAdminService configuration: " << key
                                  << std::endl;
                    }
                }
            }
            else if(strcmp(key, "VisorRegistryService") == 0)
            {
                ...
                json_object_object_foreach(visor_keeper_registry_service_conf, key, val)
                {
                    ...
                    {
                        std::cerr << "[ConfigurationManager] [chrono_grapher] Unknown VisorRegistryService configuration: " << key
                                  << std::endl;
                    }
                }
            }
            else if(strcmp(key, "Logging") == 0)
            {
                ...
                json_object_object_foreach(chrono_logging, key, val)
                {
                    ...
                    {
                        std::cerr << "[ConfigurationManager] [chrono_grapher] Unknown Logging configuration: " << key << std::endl;
                    }
                }
            }
        ...
        else if(strcmp(key, "Extractors") == 0)
        {
            ...
            {    
                        ...
                        {
                            std::cerr << "reading stroy_files_dir"<<std::endl;
                            assert(json_object_is_type(val, json_type_string));
                            GRAPHER_CONF.EXTRACTOR_CONF.story_files_dir = json_object_get_string(val);
                        }   
                        else
                        {
                            std::cerr << "[ConfigurationManager] [chrono_grapher] Unknown extractor configuration " << key << std::endl;
                        }
            }
        }
        else
