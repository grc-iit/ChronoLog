...
public:
    PlayerRegistrationMsg(PlayerIdCard const& id_card = PlayerIdCard{}, ServiceId const& service_id = ServiceId{})
...
{
    return std::string("PlayerRegistrationMsg{") + to_string(msg.getPlayerIdCard()) + std::string("}{admin:") +
           to_string(msg.getAdminServiceId()) + "}";
}
