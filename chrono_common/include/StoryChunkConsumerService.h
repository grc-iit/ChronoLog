...
public:
    ...
    static StoryChunkConsumerService*
    CreateChunkConsumerService(tl::engine &tl_engine, uint16_t service_provider_id, StoryChunkIngestionQueue &ingestion_queue)
    {
    ...
    void is_receiver_available(tl::request const &request)
    {
    ...
    void receive_story_chunk(tl::request const &request, tl::bulk &b)
    {
        try
        {
            ...
            LOG_DEBUG("[StoryChunkConsumerService] StoryChunk recording RPC invoked, ThreadID={}", tl::thread::self_id());
            ...
            LOG_DEBUG("[StoryChunkConsumerService] Bulk memory prepared, size: {}, ThreadID={}", mem_vec.size()
                      , tl::thread::self_id());
            tl::engine tl_engine = get_engine();
            LOG_DEBUG("[StoryChunkConsumerService] Engine addr: {}, ThreadID={}", (void*)&tl_engine
                      , tl::thread::self_id());
            ...
            b.on(ep) >> local;
            LOG_DEBUG("[StoryChunkConsumerService] Received {} bytes of StoryChunk data, ThreadID={}", b.size()
                      , tl::thread::self_id());
            ...
            LOG_INFO("[StoryChunkConsumerService] Deserialization took {} us, ThreadID={}",
                    std::chrono::duration_cast <std::chrono::nanoseconds>(end - start).count() / 1000.0
                     , tl::thread::self_id());
#endif
            LOG_DEBUG("[StoryChunkConsumerService] StoryChunk received: StoryId {} StartTime {} eventCount {} ThreadID={}"
                      , story_chunk->getStoryId(), story_chunk->getStartTime(), story_chunk->getEventCount()
                      , tl::thread::self_id());

            request.respond(b.size());
            LOG_DEBUG("[StoryChunkConsumerService] StoryChunk recording RPC responded {}, ThreadID={}", b.size()
                      , tl::thread::self_id());
...
private:
    StoryChunkConsumerService(tl::engine &tl_engine, uint16_t service_provider_id, StoryChunkIngestionQueue &ingestion_queue)
            : tl::provider <StoryChunkConsumerService>(tl_engine, service_provider_id), theIngestionQueue(ingestion_queue)
    {
