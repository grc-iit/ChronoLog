...
chl::KeeperConfiguration::~KeeperConfiguration()
{
    if(extraction_module_conf != nullptr)
    {
        delete extraction_module_conf;
    }
}
//////////////
...
int chronolog::KeeperConfiguration::parseJsonConf(json_object* json_conf)
{
    json_object_object_foreach(json_conf, key, val)
    {
        ...
        {
            assert(json_object_is_type(val, json_type_object));
            json_object* extraction_conf_json_object = json_object_object_get(json_conf, "ExtractionModule");
            if(extraction_module_conf != nullptr)
            {
                delete extraction_module_conf;
            }
            extraction_module_conf = new chl::ExtractionModuleConfiguration(extraction_conf_json_object);
        }
...
chl::GrapherConfiguration::~GrapherConfiguration()
{
    if(extraction_module_conf != nullptr)
    {
        delete extraction_module_conf;
...
std::string& chl::KeeperConfiguration::to_string(std::string& a_string) const
{
    a_string += "[CHRONO_KEEPER_CONFIGURATION: RECORDING_GROUP: " + std::to_string(RECORDING_GROUP) +
                ...
                (extraction_module_conf != nullptr ? extraction_module_conf->to_string(a_string)
                                                   : std::string("ExtractionModuleConf { }")) +
                "]";
...
std::string& chl::GrapherConfiguration::to_string(std::string& a_string) const
{
    a_string += "[CHRONO_GRAPHER_CONFIGURATION: RECORDING_GROUP: " + std::to_string(RECORDING_GROUP) +
                ", KEEPER_GRAPHER_DRAIN_SERVICE_CONF: " + KEEPER_GRAPHER_DRAIN_SERVICE_CONF.to_String() +
                ", DATA_STORE_ADMIN_SERVICE_CONF: " + DATA_STORE_ADMIN_SERVICE_CONF.to_String() +
                ", VISOR_REGISTRY_SERVICE_CONF: " + VISOR_REGISTRY_SERVICE_CONF.to_String() +
                ", LOG_CONF: " + LOG_CONF.to_String() + ", DATA_STORE_CONF: " + DATA_STORE_CONF.to_String() +
...
std::string& chl::PlayerConfiguration::to_string(std::string& a_string) const
{
    a_string += "[CHRONO_PLAYER_CONFIGURATION: RECORDING_GROUP: " + std::to_string(RECORDING_GROUP) +
                ", DATA_STORE_ADMIN_SERVICE_CONF: " + DATA_STORE_ADMIN_SERVICE_CONF.to_String() +
...
void chl::ConfigurationManager::LoadConfFromJSONFile(const std::string& conf_file_path)
{
    ...
    json_object_object_foreach(root, key, val)
    {
        if(strcmp(key, "clock") == 0)
        {
            ...
            {
                std::cerr << "[ConfigurationManager] Error while parsing configuration file " << conf_file_path.c_str()
                          << ". Clock configuration is not found or is not an object." << std::endl;
                exit(chronolog::CL_ERR_INVALID_CONF);
            }
