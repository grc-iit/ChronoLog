...
int main(int argc, char**argv)
{
    ...
    LOGI("Vector of streams created");
    ...
    for(int j = 0; j < nPorts; j++)
    {
        ...
        LOGI("Engine created and running@{} ...", std::string(myEngine.self()));
                                                                                           ...
                                                                                           , std::vector <std::byte> &data)
        {
            ...
            LOGI("Received {} bytes of data in send/recv mode", data.size());
        ...
        {
            LOGI("RDMA rpc invoked");
            /*std::cout << "RDMA rpc invoked" << std::endl;*/
            tl::endpoint ep = req.get_endpoint();
            LOGI("endpoint obtained");
            ...
            LOGI("RDMA memory prepared, size: {}", mem_vec.size());
            ...
            LOGI("RDMA memory exposed");
            /*std::cout << "RDMA memory exposed" << std::endl;*/
            b.on(ep) >> local;
            LOGI("Received {} bytes of data in RDMA mode", b.size());
