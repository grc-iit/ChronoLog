...
double cpu_base_frequency()
{
    ...
    for(std::string line; getline(cpuinfo, line);)
    {
        ...
        if(m.size() == 2)
        {
            Logger::getLogger()->info("[Common] CPU freq obtained from /proc/cpuinfo: {} GHz", std::stod(m[1]));
            return std::stod(m[1]);
        }
    }
    double freq = 3.8; // base clock of Ryzen 7 5700G is 3.8GHz
    Logger::getLogger()->error("[Common] Fail to get CPU freq from /proc/cpuinfo, returning hard-coded freq {} GHz"
                               , freq);
    return freq;
}
...
void printVectorStats(std::vector <T> v)
{
    ...
    Logger::getLogger()->info("[Common] Min/Max/Mean/Median/Stddev (ns)\n{}\t{}\t{}\t{}\t{}", min, max, mean, median
                              , stdev);
}
...
void writeListToFile(T*clock_list, int len, std::string &fname)
{
    ...
    if(!outFile.is_open())
    {
        Logger::getLogger()->error("[Common] Failed to open output file.");
        exit(1);
    }
