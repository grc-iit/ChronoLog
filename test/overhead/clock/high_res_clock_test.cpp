...
int main(int argc, char*argv[])
{
     ...
     */
    {
        ...
        LOG_INFO("[HighResClockTest] Test with __builtin_ia32_rdtsc() calls: ");
        LOG_INFO("[HighResClockTest] CPU_GHZ_INV: {}", CPU_GHZ_INV);
        printVectorStats(duration_list);
        LOG_INFO("[HighResClockTest] Resolution: N/A");
     ...
     */
    {
        ...
        std::vector <double> duration_list;
        convert_ull_to_double_vec(clock_list, test_reps, duration_list);
        LOG_INFO("[HighResClockTest] Test with __builtin_ia32_rdtscp() calls on processor {}: ", proc_id);
        LOG_INFO("[HighResClockTest] CPU_GHZ_INV: {}", CPU_GHZ_INV);
        printVectorStats(duration_list);
     ...
     */
    {
        ...
        struct timespec res;
        LOG_INFO("[HighResClockTest] Test with clock_gettime(CLOCK_MONOTONIC) calls: ");
        printVectorStats(duration_list);
        // writeVectorStatsToFile(duration_list);
        sort(duration_list.begin(), duration_list.end());
        ...
        LOG_INFO("[HighResClockTest] Resolution (ns): {}", res.tv_sec * 1e9 + res.tv_nsec);
        free(clock_list);
    }

    {
        ...
        convert_timespec_to_ull_vec(clock_list, test_reps, duration_list);
        struct timespec res;
        LOG_INFO("[HighResClockTest] Test with clock_gettime(CLOCK_MONOTONIC_RAW) calls: ");
        printVectorStats(duration_list);
        sort(duration_list.begin(), duration_list.end());
     ...
     */
    {
        ...
        LOG_INFO("[HighResClockTest] Test with std::chrono::high_resolution_clock::now() calls (count as late as possible): ");
        printVectorStats(duration_list);
        sort(duration_list.begin(), duration_list.end());
        fout << "C++11(alap)\t";
        printStatLine(fout, duration_list);
        LOG_INFO("[HighResClockTest] Resolution (ns): {}", (double)std::chrono::high_resolution_clock::period::num /
...

    {
        ...
        std::vector <double> duration_list;
        convert_ull_to_double_vec(clock_list, test_reps, duration_list);
        LOG_INFO("[HighResClockTest] Test with std::chrono::high_resolution_clock::now() calls (count immediately): ");
        printVectorStats(duration_list);
        sort(duration_list.begin(), duration_list.end());
        fout << "C++11(imme)\t";
        printStatLine(fout, duration_list);
