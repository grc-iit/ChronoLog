...
int main(int argc, char* argv[])
{
     ...
     */
    {
        ...
        std::vector<double> duration_list;
        convert_ull_to_double_vec(clock_list, test_reps, duration_list);
        std::cout << "[HighResClockTest] Test with __builtin_ia32_rdtscp() calls on processor: " << proc_id
                  << std::endl;
        std::cout << "[HighResClockTest] CPU_GHZ_INV: " << CPU_GHZ_INV << std::endl;
        printVectorStats(duration_list);
     ...
     */
    {
        ...
        std::cout << "[HighResClockTest] Test with std::chrono::high_resolution_clock::now() calls (count as late as "
                     "possible): "
                  << std::endl;
        printVectorStats(duration_list);
        sort(duration_list.begin(), duration_list.end());
        fout << "C++11(alap)\t";
        printStatLine(fout, duration_list);
        std::cout << "[HighResClockTest] Resolution (ns): "
