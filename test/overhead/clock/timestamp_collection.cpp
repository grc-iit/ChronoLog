...
void clock_gettime_thread(void* args)
{
    ...
    std::cout << "[TimestampCollection] Thread ID: " << thread_id
              << " completed execution. Timestamps saved to file: " << fname << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    int sleep_ns = 0;
    if(argc > 1)
        n_threads = std::strtol(argv[1], nullptr, 10);
    if(argc > 2)
        sleep_ns = std::strtol(argv[2], nullptr, 10);
