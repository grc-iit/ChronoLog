...
verify_components_running() {
  ...
  for role in visor keeper grapher player; do
    ...
    visor)
      hfile="$VISOR_HOSTS"
      pattern="chrono-visor"
      ;;
    keeper)
      hfile="$KEEPER_HOSTS"
      pattern="chrono-keeper"
      ;;
    grapher)
      hfile="$GRAPHER_HOSTS"
      pattern="chrono-grapher"
      ;;
    player)
      hfile="$PLAYER_HOSTS"
      pattern="chrono-player"
      ;;
    esac
...
event_count_for_size() {
  ...
  if ((esz <= 8192)); then
    base=2000
  elif ((esz <= 131072)); then
    base=1000
  elif ((esz <= 262144)); then
    base=500
  elif ((esz <= 524288)); then
    base=250
  else
    base=125
  fi
...
run_one_perf_test() {
  ...
  local specific
  specific="$(test_specific_flags "$test" "$ecnt")"
