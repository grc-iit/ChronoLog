...
// Test 3 ExtractionModule Configuration
    ...
    for( auto iter = module_config.extractors.begin(); iter != module_config.extractors.end(); ++iter)
    {
      ...
      {
         std::cerr<< "\n [ExtractionModuleConfiguration] Test borrowed reference stage:" <<" failed to retain extractor_json_object for"<<(*iter).first << std::endl;
         return (-1);
      }
    }
    ...
    std::cout<< "\n [ExtractionModuleConfiguration] Test borowed reference stage:"<<" passed :"<< module_config.to_string(log_string) << std::endl;
