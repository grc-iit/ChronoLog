...
kernel_clock_test_start(void)
{
    ...
    uint64_t* variances;
    uint64_t* min_values;
    uint64_t max_dev = 0, min_time = 0, max_time = 0, prev_min = 0, tot_var = 0, max_dev_all = 0, var_of_vars = 0,
             var_of_mins = 0;
    uint64_t overall_min = 0, overall_max = 0, overall_mean = 0, local_mean_acc = 0;
    uint64_t overall_dev_sq = 0, local_dev_sq = 0;

    printk(KERN_INFO "Loading kernel_clock_test module...\n");
    ...
    for(j = 0; j < BOUND_OF_LOOP; j++) { local_mean_acc += mean_calc(times[j], SIZE_OF_STAT); }
    overall_mean = local_mean_acc / BOUND_OF_LOOP;
    for(j = 0; j < BOUND_OF_LOOP; j++)
    {
        for(i = 0; i < SIZE_OF_STAT; i++) { times[j][i] = abs(times[j][i] - overall_mean); }
        local_dev_sq = dev_sq_calc(times[j], SIZE_OF_STAT);
        overall_dev_sq += local_dev_sq;
        printk(KERN_ERR "loop_size:%d >>>> dev square(cycles): %llu", j, local_dev_sq);
    }
    ...
    for(j = 0; j < BOUND_OF_LOOP; j++) { kfree(times[j]); }
    kfree(times);
    kfree(variances);
    kfree(min_values);
    return 0;
