...
int main(int argc, char** argv)
{
    if(argc < 3)
    {
        cout << "Usage: ./sync_overhead_test mode #threads" << endl
             << "mode = 0: no sync" << "mode = 1: use std::mutex" << endl
             << "mode = 2: use std::atomic" << endl
             << "mode = 3: use TSC" << endl
    ...
    for(int i = 0; i < num_thrds; i++) { thrds[i] = thread(thread_func); }

    this_thread::sleep_for(std::chrono::seconds(TEST_DURATION));

    stop = true;
    for(int i = 0; i < num_thrds; i++) { thrds[i].join(); }
