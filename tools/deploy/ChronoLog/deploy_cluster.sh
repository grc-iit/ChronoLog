...
generate_conf_for_each_recording_group() {
  ...
  for i in $(seq 1 ${NUM_RECORDING_GROUP}); do
    ...
    jq ".chrono_keeper.ExtractionModule.extractors.csv_extractor.csv_archive_dir = \"${OUTPUT_DIR}\"" "${CONF_FILE}.${i}" >${CONF_DIR}/temp.json && mv ${CONF_DIR}/temp.json "${CONF_FILE}.${i}"
